import albumDharma from "../images/album_dharma_endpoint.jpeg";
import albumMinistry from "../images/album_ministry_of_apparatus.jpeg";
import albumSacred from "../images/album_sacred_dope.jpeg";
import albumSolitary from "../images/album_solitary_oasis.jpeg";
import albumSolmusic from "../images/album_solmusic.jpeg";
import albumSpaseBaby from "../images/album_spase_baby.jpeg";
import MemoryHeap from "../images/memory-heapCove.jpg";
import albumWinnersSol from "../images/album_happiness.jpeg";
import albumWinnersIono from "../images/sub_iono.jpg";
import AlbumGrovemaster from "../images/album_galactic_grovemaster.jpg";
import AlbumLoveUkraine from "../images/album_peace_love_ukraine.jpg";
import AlbumTurnMeOn from "../images/album_turn_me_on.jpg";
import AlbumUniverseBeyond from "../images/album_universe_beyond.jpg";
import albumFreeYourGhostages from "../images/album_free_your.jpg";
import albumBetterPlanet from "../images/album_better_planet.jpg";
import FeedYourWoolf from "../images/album_feed_woolf.jpeg";
import hypnoticRift from "../images/hypnotic_rift.jpg";
import cosmicBalance from "../images/cosmic_balance.jpg";
import progressiveTransition from "../images/progressive_transition.jpg";
import ritualReboot from "../images/ritual_reboot.jpg";
import modernVimanas from "../images/modern_vimanas.jpg";
import advancedAnomaly from "../images/advanced_anomaly.jpg";
import mandoviDreams from "../images/mandovi_dreams.jpg";
import metaphysicalFrequencies from "../images/metaphysical_frequencies.jpg";

export interface Album {
  img: string;
  title: string;
  date: string;
  label: string;
  youtube: string;
}

export const albumData: Album[] = [
  {
    img: metaphysicalFrequencies,
    title: "Metaphysical Frequencies",
    date: "June 27, 2025",
    label: "Ovnimoon",
    youtube: "https://www.youtube.com/watch?v=RSvtOfX27fE&list=PLKf7fie1PLSaz8rY2xGfCYmqiE6feGKr8",
  },
  {
    img: mandoviDreams,
    title: "Mandovi Dreams",
    date: "May 23, 2025",
    label: "Ovnimoon",
    youtube: "https://www.youtube.com/watch?v=plIyQZR69x4&list=PLKf7fie1PLSaXat4POcHLDfDjib0eCLwd&index=2",
  },
  {
    img: hypnoticRift,
    title: "Hypnotic Rift",
    date: "May 2, 2025",
    label: "Ovnimoon",
    youtube: "https://www.youtube.com/watch?v=YF-8oUAkqMc&list=PLKf7fie1PLSYrfafMrKkuK5IbQbO9700c",
  },
  {
    img: advancedAnomaly,
    title: "Advanced Anomaly",
    date: "March 21, 2025",
    label: "Ovnimoon",
    youtube: "https://www.youtube.com/watch?v=ci7KjFqE7d8&list=PLKf7fie1PLSaMM8eOGImnimgNNFg9tGw4&index=2",
  },
  {
    img: modernVimanas,
    title: "Modern Vimanas",
    date: "July 26, 2024",
    label: "Ovnimoon",
    youtube: "https://www.youtube.com/watch?v=E6CWHHfzaJ4&list=PLKf7fie1PLSaIFBX7pjzE7SUKcJj9HRwU",
  },
  {
    img: ritualReboot,
    title: "Ritual Reboot",
    date: "June 28, 2024",
    label: "Ovnimoon",
    youtube: "https://youtu.be/SeUX4JVinfE",
  },
  {
    img: progressiveTransition,
    title: "Progressive Transition",
    date: "June 7, 2024",
    label: "Ovnimoon",
    youtube: "https://youtu.be/pYzApnX_K6c",
  },
  {
    img: cosmicBalance,
    title: "Cosmic Balance",
    date: "December 1, 2023",
    label: "Spiral Trax",
    youtube: "https://www.youtube.com/watch?v=fH4ekyymTo8&list=PLKf7fie1PLSZ3R7XfnGkEbLlOxMzgx9IF",
  },
  {
    img: albumFreeYourGhostages,
    title: "Free Your Ghostages",
    date: "October 19, 2023",
    label: "Spiral Trax",
    youtube:
      "https://www.youtube.com/watch?v=1Bpa4xxIs2U",
  },
  {
    img: albumBetterPlanet,
    title: "Better as a Planet",
    date: "September 27, 2023",
    label: "Fractal Records",
    youtube:
      "https://www.youtube.com/watch?v=Iu6gVZZZm68",
  },
  {
    img: FeedYourWoolf,
    title: "Feed Your Wolf",
    date: "November 13, 2023",
    label: "Sentimony Records",
    youtube:
      "https://www.youtube.com/watch?v=onltqi2mMK8  ",
  },

  {
    img: AlbumUniverseBeyond,
    title: "The Universe Beyond",
    date: "May 11, 2023",
    label: "Sol Music",
    youtube:
      "https://www.youtube.com/watch?v=4erMKmKIGC8&list=PLKf7fie1PLSbx8UpA3_o_9KPKT7DK3iQw&index=2",
  },

  {
    img: AlbumLoveUkraine,
    title: "KimON",
    date: "May 12, 2023",
    label: "Skygravity records",
    youtube: " https://www.youtube.com/watch?v=h_UlZpQy7mA",
  },
  {
    img: AlbumGrovemaster,
    title: "Galactic Groovemaster",
    date: "March 31, 2023",
    label: "Spiral Trax",
    youtube: "https://www.youtube.com/watch?v=ZIX21g6lsQU",
  },
  {
    img: albumDharma,
    title: "Dharma endpoint",
    date: "September 19, 2021",
    label: "Balcosmos",
    youtube:
      "https://www.youtube.com/playlist?list=PLKf7fie1PLSa0tsBfN2D5RwpnbFwYyN2E",
  },

  {
    img: albumSolitary,
    title: "Solitary Oasis",
    date: "January 6, 2023",
    label: "GreenTree Records",
    youtube:
      "https://www.youtube.com/playlist?list=PLKf7fie1PLSa0tsBfN2D5RwpnbFwYyN2E",
  },
  {
    img: albumSacred,
    title: "albumSacred",
    date: "March 26, 2021",
    label: "Balcosmos",
    youtube:
      "https://www.youtube.com/playlist?list=PLKf7fie1PLSbgKS4RFN0xCLONCzlhynw8",
  },
  {
    img: albumMinistry,
    title: "Pleasure Controller",
    date: "November 17, 2022",
    label: "UP!NOIZE Records",
    youtube: "https://www.youtube.com/watch?v=WtwFV5bYviI",
  },
  {
    img: albumSpaseBaby,
    title: "Ringing blizzard",
    date: "November 10, 2021",
    label: "Space Baby Records",
    youtube: "https://www.youtube.com/playlist?list=PLKf7fie1PLSay02EG9UBef25r_uyTIKL-",
  },
  {
    img: albumSolmusic,
    title: "Projection Of Reality ",
    date: "August 26, 2022",
    label: "Sol Music",
    youtube:
      "https://www.youtube.com/playlist?list=PLKf7fie1PLSZalWJpEogk9yZ_yBOrbpth",
  },
  {
    img: AlbumTurnMeOn,
    title: "Turn Me On",
    date: "May 26, 2023",
    label: "Spiral Trax",
    youtube: "https://youtu.be/6uT0PK7YdzM",
  },

  {
    img: MemoryHeap,
    title: "Memory heap",
    date: "February 3, 2023",
    label: "Interface Records",
    youtube:
      "https://soundcloud.com/balcosmos/balcosmos-balcosmos-memory-heap-interface-records",
  },
  {
    img: albumWinnersSol,
    title: "Happiness Is your Decision",
    date: "December 26, 2021",
    label: "Sol Music",
    youtube: "https://www.youtube.com/watch?v=ctt1KGXUhNs",
  },
  {
    img: albumWinnersIono,
    title: "Insect in a Sect",
    date: "August 12, 2021",
    label: "Iono Music",
    youtube: "https://www.youtube.com/watch?v=9GaCzajhIKw",
  },
];
